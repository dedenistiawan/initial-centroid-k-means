...
# Compute the Mean of the Selected Principal Components
mean_values <- colMeans(pca_data)
...
data_vectors <- calculate_vector(mean_values, pca_data)
initial_center <- pca_data[which.max(data_vectors), ]
...
for (k in 2:K) {
  distances <- lapply(cluster_centers, function(center) {
    ...
    calculate_vector(center, pca_data)
  })
  distances <- do.call(cbind, distances)
  sum_distances <- rowSums(distances)
  next_center <- pca_data[which.max(sum_distances), ]
...
clusters <- assign_cluster(pca_data, cluster_centers)
...
colnames(initial_centers) <- c("PC1", "PC2")
...
# Create scatter plot using PC1 and PC2 with clusters and initial cluster center
ggplot(iris, aes_string(x = "PC1", y = "PC2", color = "factor(Cluster)")) +
  geom_point(size = 3) +
  geom_point(data = initial_centers_df, aes_string(x = "PC1", y = "PC2"), color = "red", size = 5, shape = 4) +
  ...
  geom_text(data = initial_centers_df, aes_string(x = "PC1", y = "PC2", label = "label"), vjust = -1, color = "red") +
