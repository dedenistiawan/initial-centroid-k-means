...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("normalize", "Normalization Method:", choices = c("None", "Min-Max", "Z-score"), selected = "None"),
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Davies-Bouldin Index", verbatimTextOutput("daviesBouldinIndexOutput"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$clusterBtn, {
    ...
    # Conditionally apply Normalization to the Data
    if (input$normalize == "Min-Max") {
      numeric_data <- as.data.frame(lapply(numeric_columns, normalize_minmax))
    } else if (input$normalize == "Z-score") {
      numeric_data <- as.data.frame(lapply(numeric_columns, normalize_zscore))
    } else {
    ...
    davies_bouldin_index <- index.DB(numeric_data[, -ncol(numeric_data)], kmeans_result$cluster)$DB
    ...
    output$daviesBouldinIndexOutput <- renderText({
      paste("Davies-Bouldin Index:", davies_bouldin_index)
    })
