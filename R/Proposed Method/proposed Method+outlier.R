...
data(iris)
original_data <- iris  # Keep a copy of the original data
...
remove_outliers <- function(data) {
  ...
  for (col in 1:ncol(data)) {
    low_threshold <- Q1[col] - 1.5 * IQR[col]
    high_threshold <- Q3[col] + 1.5 * IQR[col]
    outlier_free_data <- outlier_free_data[outlier_free_data[, col] >= low_threshold & outlier_free_data[, col] <= high_threshold, ]
  }
...
# Create a new dataframe to match clusters with original data
data_rownames <- rownames(data)
clustered_data <- original_data[data_rownames, ]
clustered_data$Cluster <- clusters

# Calculate mean values of each cluster
cluster_means <- aggregate(. ~ Cluster, data = clustered_data[, -5], mean)
...
true_labels <- as.integer(original_data$Species)  # Convert species to numeric
predicted_labels <- rep(NA, nrow(original_data))
predicted_labels[data_rownames] <- kmeans_result$cluster

# Print diagnostic information
cat("True Labels Length:", length(true_labels), "\n")
cat("Predicted Labels Length:", length(predicted_labels), "\n")
cat("Complete Indices Length:", length(complete_indices), "\n")
cat("Complete Indices:", complete_indices, "\n")
cat("True Labels (Complete):", true_labels[complete_indices], "\n")
cat("Predicted Labels (Complete):", predicted_labels[complete_indices], "\n")

# Ensure no NA values in the true_labels and predicted_labels vectors used for Rand Index
complete_indices <- which(!is.na(predicted_labels))
rand_index <- rand.index(true_labels[complete_indices], predicted_labels[complete_indices])
cat("Rand Index:", rand_index, "\n")

# Display the first few rows of the updated dataset
print(head(clustered_data))
...
ggplot(clustered_data, aes_string(x = attribute1, y = attribute2, color = "factor(Cluster)")) +
  geom_point(size = 3) +
